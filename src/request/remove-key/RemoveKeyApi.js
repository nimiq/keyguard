...
class RemoveKeyApi extends TopLevelApi { // eslint-disable-line no-unused-vars
    ...
    async onRequest(request) {
        /** @type {KeyguardRequests.ParsedSimpleRequest} */
        ...
        this._removeKeyPassphraseBox.on(PassphraseBox.Events.SUBMIT, async p => {
            if (await this._tryRetrieveAndSetKey(p, /** @type {PassphraseBox} */ (this._removeKeyPassphraseBox))) {
                ...
                /** @type {KeyguardRequests.SimpleResult} */
                const result = {
                    success: true,
                };
                this.resolve(result);
    ...
    async _tryRetrieveAndSetKey(phrase, box) {
        ...
        try {
            ...
            const key = await KeyStore.instance.get(
                /** @type {KeyguardRequests.ParsedSimpleRequest} */ (this.parsedRequest).keyInfo.id,
                passphraseBuffer,
            );
            if (!key) {
            ...
            this._setKey(
                key,
                /** @type {KeyguardRequests.ParsedSimpleRequest} */ (this.parsedRequest).keyInfo.encrypted,

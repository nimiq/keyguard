...
function DummyData() {
    ...
    const keyInfos = () => [
        new KeyInfo(
            ...
            false,
            false,
            new Nimiq.Entropy(keys[0]).toExtendedPrivateKey().derivePath(Constants.DEFAULT_DERIVATION_PATH).toAddress().serialize(),
        ),
        new KeyInfo(
            hashes()[1],
            ...
            true,
            false,
            Nimiq.PublicKey.derive(new Nimiq.PrivateKey(keys[1])).toAddress().serialize(),
        ),
    ];

...
describe('BitcoinKey', () => {
    ...
    it('can derive addresses (BIP49)', () => {
        const key = new Key(Dummy.secrets[0]);
        const btcKey = new BitcoinKey(key);
        // Addresses generated by https://iancoleman.io/bip39/
        const address0 = '2NECqMUWguYyGZ5ivvwf7bxdPjveDUJdVgE';
        const address1 = '2NFaZE3syms5ELVQGt3NW7hHEB2kLSUbkaE';
        expect(btcKey.deriveAddress(`${BitcoinConstants.ACCOUNT_DERIVATION_PATH.BIP49.TEST}/0/0`)).toEqual(address0);
        expect(btcKey.deriveAddress(`${BitcoinConstants.ACCOUNT_DERIVATION_PATH.BIP49.TEST}/0/1`)).toEqual(address1);
    });

    it('can derive addresses (BIP84)', () => {
        const key = new Key(Dummy.secrets[0]);
        const btcKey = new BitcoinKey(key);
        // Addresses generated by https://iancoleman.io/bip39/
        const address0 = 'tb1qyeraymz6cywnwrlhmkshkjm2pn0qtj3zf6demn';
        const address1 = 'tb1q646hj7ylszarnk4urqw4jnzud8mzq8jye48dnm';
        expect(btcKey.deriveAddress(`${BitcoinConstants.ACCOUNT_DERIVATION_PATH.BIP84.TEST}/0/0`)).toEqual(address0);
        expect(btcKey.deriveAddress(`${BitcoinConstants.ACCOUNT_DERIVATION_PATH.BIP84.TEST}/0/1`)).toEqual(address1);
    });

    it('can derive extended public key', () => {
        const key = new Key(Dummy.secrets[0]);
        const btcKey = new BitcoinKey(key);
        // XPubs generated by https://iancoleman.io/bip39/
        const bip49xpub = 'upub5EpmxeG58t817P5mHsjEsxrysgBcE9fXrfEL2H4mSaP3cGrf9PVrVDw6uHAb8Veswj9AUq4iuF8xcbm24kLGa5emr1S3WXAwbbioioGiqAF';
        const bip84xpub = 'vpub5ZLdfP5ZLnyhRGainMG1tqyq2ihJsLABet5727n3HHMQyBD4ytaGvaKiA7jUVDNWJrD192dJyKpdFbNnAZRAGkQ1r8Jpz1q2r261pXKceTh';
        expect(btcKey.deriveExtendedPublicKey(BitcoinConstants.ACCOUNT_DERIVATION_PATH.BIP49.TEST)).toEqual(bip49xpub);

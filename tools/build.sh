...
# Abstraction, because e.g. MacOS does not have sha256sum by default
sha256() {
    # The hash command is a simple way to detect if a command is available in the $PATH
    if hash sha256sum 2>/dev/null; then # Linux
        sha256sum "$@"
    elif hash shasum 2>/dev/null; then # MacOS
        shasum -a 256 "$@"
    else
        echo "No suitable sha256 hash function found!"
        exit 1
    fi
}
